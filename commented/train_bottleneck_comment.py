...
def _main():
    ...
    classes_path = 'model_data/coco_classes.txt'#類別文件
    anchors_path = 'model_data/yolo_anchors.txt'#anchor box的配置文件
    class_names = get_classes(classes_path)()#類別列表
    num_classes = len(class_names)#類別數
    anchors = get_anchors(anchors_path)#anchor box類別

    input_shape = (416,416) # multiple of 32, hw#輸入圖像
    ...
    checkpoint = ModelCheckpoint(log_dir + 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5',
        monitor='val_loss', save_weights_only=True, save_best_only=True, period=3)#只储存weights
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=3, verbose=1)
#reduce_lr：當評價指標不在提升時，減少學習率，每次減少10%，當驗證損失值，持續3次未減少時，則終止訓練。
    early_stopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=10, verbose=1)#early_stopping：當驗證集損失值，連續增加小於0時，持續10個epoch，則終止訓練。

    val_split = 0.1#訓練和驗證的比率
    ...
    if True:
        ...
        model.save_weights(log_dir + 'trained_weights_stage_0.h5')#存儲訓練參數
        ...
        model.save_weights(log_dir + 'trained_weights_stage_1.h5')#存儲訓練參數
    ...
    if True:#全部訓練
        ...
        model.save_weights(log_dir + 'trained_weights_final.h5')#儲存最終參數

    # Further training if needed.


def get_classes(classes_path):#載入類別文件,讀取文件中所有類別,生成list
    '''loads the classes'''
...
def get_anchors(anchors_path):#載入anchor box組合文件
...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    h, w = input_shape#尺寸
    num_anchors = len(anchors)#anchor box數量

    y_true = [Input(shape=(h//{0:32, 1:16, 2:8}[l], w//{0:32, 1:16, 2:8}[l], \#YOLO的三種尺度,每個尺度的anchor box數,類別數+邊框+置信度
    ...
    if load_pretrained:#加載預訓練模型
        ...
        if freeze_body in [1, 2]:
            ...
            for i in range(num): model_body.layers[i].trainable = False#將其他層的訓練關閉
...
def data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, random=True, verbose=False):
    ...
    n = len(annotation_lines)#annotation_lines:所有的圖片名稱
    i = 0
    while True:
        image_data = []
        box_data = []
        for b in range(batch_size):
            if i==0 and random:
                np.random.shuffle(annotation_lines)#隨機排列圖片順序
            image, box = get_random_data(annotation_lines[i], input_shape, random=random)#獲得圖片和邊框
            image_data.append(image)#添加圖片
            box_data.append(box)#添加邊框
            i = (i+1) % n
        image_data = np.array(image_data)
        if verbose:
            print("Progress: ",i,"/",n)
        box_data = np.array(box_data)
        y_true = preprocess_true_boxes(box_data, input_shape, anchors, num_classes)#y_true是三個預測特徵的列表
        yield [image_data, *y_true], np.zeros(batch_size)

def data_generator_wrapper(annotation_lines, batch_size, input_shape, anchors, num_classes, random=True, verbose=False):#用於條件檢查
...
def bottleneck_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, bottlenecks):
    ...
    while True:
        ...
        for b in range(batch_size):
            _, box = get_random_data(annotation_lines[i], input_shape, random=False, proc_img=False)#獲取邊框
            box_data.append(box)#添加邊框
